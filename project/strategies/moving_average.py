...
class MovingAverageStrategy:
    ...
    def make_signal(self,signal,price,buynhold=None):
        return { 
            "signal" : signal,
            "price" : price,
            "buy_and_hold_return_pct" : buynhold
        }
    ...
    def on_event(self,event):
        ...
        if(short<long):
            ...
            return self.make_signal("BUY",current_price)
        else:
            buynhold = (current_price-self.start_price) / self.start_price * 100
            
            if self.bought is None:
                return self.make_signal("SELL_IGNORED",current_price,buynhold)
            else:
                sold = (current_price-self.bought) / self.bought *100

            return self.make_signal("SELL",current_price,buynhold)
